...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="run all experiments on a single machine",
)
...
def main(
    ...
    try:
        ...
        while len(ready_refs) < len(task_refs):
            this_ready_refs, rest_refs = ray.wait(
                rest_refs, num_returns=1, timeout=None, fetch_local=True
            )
